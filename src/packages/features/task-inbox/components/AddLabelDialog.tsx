...
export default function AddLabelDialog({
  ...
  const [
    createLabelFn,
    { isLoading: isEditing, data: createdLabel, isSuccess: isCreateSuccessful },
  ] = useCreateLabelMutation();
  const [
    editLabelFn,
    { isLoading: isCreating, data: editedLabel, isSuccess: isEditSuccessful },
  ] = useEditLabelMutation();
  ...
  const onSubmit = (data: CreateLabelRequest) => {
    if (editId !== undefined && editId !== 0) {
      editLabelFn({ id: editId, body: data });
    } else {
      createLabelFn({
        ...data,
      });
