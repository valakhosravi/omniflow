...
import { formatFileSize } from "@/packages/features/development-ticket/utils/formatFileSize";
import { Button } from "@heroui/react";
import fetchImageFile from "@/components/food/FetchImageFile";
import ProcurementSecondCheckAdditionalDescription from "./ProcurementSecondCheckAdditionalDescription";
...
interface ProcurementSecondCheckDescriptionProps {
  requestId?: string;
...
export default function ProcurementSecondCheckDescription({
  invoiceDetails,
  refetch,
  requestStatus,
  requestId,
}: ProcurementSecondCheckDescriptionProps) {
  ...
  useEffect(() => {
    const fileAddress = invoiceDetails?.Data?.WarehouseReceiptFileAddress;
    ...
    loadFile();
  }, [invoiceDetails?.Data?.WarehouseReceiptFileAddress]);
  ...
  const formData: (
    ...
    },
    {
      ...
      title: "فایل رسید انبار",
      value: invoiceDetails?.Data?.WarehouseReceiptFileAddress ? (
        <div
          key={"warehousefile"}
  ...
  return (
    <div className="col-span-8 border border-primary-950/[.1] rounded-[20px] p-4 space-y-[24px] mb-4">
      ...
      <InvoiceDetail
        ...
        isLong={isLong}
      />
      <ProcurementSecondCheckAdditionalDescription
