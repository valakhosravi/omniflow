...
import { SubCategoryField, ContractFieldType } from "../../types/contractModel";
...
import { More, Copy } from "iconsax-reactjs";
...
export default function VariablesManager({
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (over && active.id !== over.id) {
      ...
      const updatedVariables = reorderedVariables.map((variable, index) => ({
        ...variable,
        SortOrder: index + 1,
      }));
                    ...
                    rules={{
                      ...
                      validate: (value) => {
                        ...
                        const isDuplicate = variables.some((v) => {
                          ...
                          return v.Name?.toLowerCase().trim() === normalizedValue;
                        });
                        return (
                          !isDuplicate || "این نام قبلاً استفاده شده است و باید یکتا باشد"
                        );
                      },
                    }}
