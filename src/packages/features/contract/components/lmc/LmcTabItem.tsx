...
import { ArrowDown2, ArrowUp2, Edit2, Send2, Trash } from "iconsax-reactjs";
import {
  Checkbox,
  CheckboxGroup,
  Chip,
...
export default function LmcTabItem({
  ...
  const { requestId, termDepartments } = useSelector(
    (state: RootState) => state.lmcData
  );
  ...
  const [saveTermAssignee, { isLoading: isSaving }] =
    useSaveTermAssigneeMutation();
  ...
  const handleRefere = async () => {
    ...
    await saveTermAssignee({
      ContractId: contractData.ContractId,
      BusinessKey: request.Data.BusinessKey,
      RequestId: Number(requestId),
      TermGroups: termIds.map((termId) => {
        const termDept = termDepartments.find((td) => td.termId === termId);
        return {
          TermId: termId,
          GroupKeys: termDept?.departments ?? [],
        };
      }),
    });
  };
  ...
  const buttons = [
    <CustomButton
      ...
      onPress={() => handleRefere()}
      isLoading={isSaving}
      isDisabled={termDepartments.length === 0}
    >
      ارجاع
    </CustomButton>,
    <CustomButton
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-x-2.5">
                          <CustomButton
                            ...
                            buttonSize="xs"
                            startContent={<Edit2 size={16} />}
                            onPress={() => handleRefereTerm(Number(term.TermId))}
                          >
                            ارجاع بند
                          </CustomButton>
                          <CustomButton
                          ...
                          <CustomButton
                            ...
                            onPress={() => handleDeleteTerm(Number(term.TermId))}
                            isLoading={
