...
import { setFormData } from "../../../slice/ContractDataSlice";
import { useEffect, useState, useMemo } from "react";
import { useGetContractInfoByContractIdQuery } from "../../../api/contractApi";
...
export default function PreviewOfContract() {
  ...
  const selectedContract = useSelector(
    (state: RootState) => state.contractData.selectedContract
  );
  const {
  ...
  // Fetch contract info
  const { data: contractInfo, isLoading: isLoadingContract } = useGetContractInfoByContractIdQuery(
    contractId,
  ...
  const signatureSettings: SignatureSettings = useMemo(() => {
    console.log('selectedContractFormData', selectedContractFormData)
    ...
    return {
      ...
      signerOrganizationPosition: String(fieldValues.signerOrganizationPosition || ""),
      signaturePlacement: (fieldValues.signaturePlacement as "endOfContract" | "endOfEachPage") || "endOfContract",
    };
  ...
  const handlepreviousStep = () => {
    ...
    if (selectedContractFormData) {
      localStorage.setItem('contractFormData', JSON.stringify(selectedContractFormData));
    }
    window.history.back();
  };
