...
import { useFullSaveMutation } from "../../api/contractApi";
import { FullSaveRequest, ContractClauses, ContractFieldType } from "../../types/contractModel";
import {
  FieldValueDetailsModel,
  ContractSetting,
} from "../../types/contractFieldsModel";
import { Skeleton, Switch, Checkbox } from "@heroui/react";
...
import { setcontractTitle } from "../../slice/ContractDataSlice";
...
export default function CompletingContractEntries() {
  ...
  const { template: subCategoryTemplate, isLoading: isLoadingTemplate } = useGetSubCategoryTemplate(subCategoryId || null);
  ...
  }, [contractFieldValues, fields, formData, reset]);
  ...
  const formatFieldValue = (key: string, value: any): string => {
    ...
    if (
      ...
      if (normalizedBool === "true" || normalizedBool === "1" || normalizedBool === "yes") {
        return "بله";
      }
      if (normalizedBool === "false" || normalizedBool === "0" || normalizedBool === "no") {
        return "خیر";
      }
  ...
  const onSubmit = async (data: ContractEntriesFormData) => {
    ...
    Object.entries(data).forEach(([key, value]) => {
      if (!["needsSignature", "signerCompanyName", "signerPerson", "signerOrganizationPosition", "signaturePlacement"].includes(key)) {
    ...
    if (subCategoryTemplate?.Template) {
      try {
        ...
        const processedTemplate = replacePlaceholdersInObject(parsedTemplate, formValues);
        
        // Transform to ContractClauses format
        // Template can have ContractClauses array directly or nested in the object
        const clausesArray = processedTemplate.ContractClauses || 
        ...
        if (Array.isArray(clausesArray) && clausesArray.length > 0) {
          contractClauses = clausesArray.map((clause: any, index: number) => ({
            ...
            Terms: (clause.Terms || []).map((term: any, termIndex: number) => ({
              Title: term.Title || "",
              InitialDescription: term.InitialDescription || term.FinalDescription || "",
    ...
    const contractFields = Object.entries(data)
      ...
      .filter((item): item is { ContractFieldId: number; FieldValue: string } => item !== null);
    ...
    const fieldValueDetails: FieldValueDetailsModel[] = contractFields.map((field) => ({
      ContractFieldId: field.ContractFieldId,
    ...
    try {
      ...
      if (res?.ResponseCode === 100) {
        ...
        router.push(
          `/contract/type/preview?categoryId=${categoryId}&contractId=${newContractId}`
        );
      } else {
        addToaster({
      ...
      <div className="space-y-[24px]">
        ...
        >
          ...
          <div className="flex flex-col gap-6 border-t border-primary-950/[.1] pt-6 mt-6">
            ...
            {Boolean(watchedValues.needsSignature) && <div className="grid grid-cols-2 gap-6">
              <RHFInput
                key={"signerCompanyName"}
                ...
                <div className="relative mt-[10px]">
                  <Controller
                    ...
                    render={({ field }) => (
                      ...
                      >
                        <SelectItem key="endOfContract">انتهای قرارداد</SelectItem>
                        <SelectItem key="endOfEachPage">انتهای هر صفحه</SelectItem>
                      </Select>
                    )}
                  />
                </div>
