...
import { Category, SubCategory } from "../../types/contractModel";
...
export default function NonTypeContractModal({
  ...
  const onSubmit = useCallback((data: NonTypeContractEntriesFormData) => {
    ...
    setLoading(true);
    if (isTemplate) {
      router.push(`/contract/personal-template/create?categoryId=${finalCategoryId}`);
    } else {
      const url = subCategoryId 
        ? `/contract/non-type/?categoryId=${finalCategoryId}&subCategoryId=${subCategoryId}`
        : `/contract/non-type/?categoryId=${finalCategoryId}`;
      router.push(url);
    }
  },
    [isTemplate, categoryId, selectedCategoryId, subCategoryId, dispatch, router]
  );
