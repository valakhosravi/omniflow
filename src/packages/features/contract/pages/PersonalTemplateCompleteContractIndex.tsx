...
export default function PersonalTemplateCompleteContractIndex({
  showStepper = true,
  ...
  const contractStats = useMemo(() => {
    ...
    const termsCount = clauses.reduce(
      (total, clause) => {
    ...
    const subClausesCount = clauses.reduce(
      (total, clause) => {
        const terms = Array.isArray(clause?.Terms) ? clause.Terms : [];
  ...
  const handleConfirmSubmit = async (settings: SignatureSettings) => {
    ...
    if (taskId) {
      ...
      try {
        ...
        const saveResponse = await fullSave(submissionDataWithSettings).unwrap();
    ...
    <ContractLayout>
      <div className="container mx-auto">
        ...
        <DevelopmentTicketHeader
          title={taskId ? "ویرایش قالب" : "ایجاد قالب جدید"}
        />
        ...
        <ContractActionToolbar
          ...
          onSaveAsTemplate={
            isTemplatePage
              ? async (data?: SaveTemplateData | SaveTemplateWithFieldsFormData) => {
                  if (data) {
