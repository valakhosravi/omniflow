...
import { ArrowDown2, ArrowUp2, Task } from "iconsax-reactjs";
    ...
    >
      <div className={`${isOpen ? "space-y-4" : ""} w-full`}>
        <div className="flex gap-x-3">
          ...
          >
            <Task className="text-primary-950" />
          </div>
          <div>
      ...
      <div className="shrink-0 ml-2 mt-[30.5px] transition-transform duration-300 ease-in-out">
        {isOpen ? (
          <ArrowUp2 className="size-[12px] text-secondary-950" />
        ) : (
          <ArrowDown2 className="size-[12px] text-secondary-950" />
        )}
      </div>
    </div>
  );
