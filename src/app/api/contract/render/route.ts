...
import fs from "fs";
import ejs from "ejs";
import { toFaDigits, mapContentFa, removeInlineFontFamily } from "@/utils/contractPdfUtils";
...
export async function POST(req: NextRequest) {
  try {
    ...
    const approvalHistory = body.approvalHistory || body.ApprovalHistory || null;
    ...
    const approvalHistoryProcessed = approvalHistory && Array.isArray(approvalHistory) 
      ? approvalHistory.map((item: any) => {
          // Map EntityTypeId to EntityTypeName: 1 = ماده (Clause), 2 = بند (Term)
          const entityTypeName = item.EntityTypeId === 1 
            ? "ماده" 
            : item.EntityTypeId === 2 
            ? "بند" 
            : "";
...
<head>
  ...
  <style>
    ...
    * {
    ...
    .clause {
      margin-bottom: 15mm;
      page-break-inside: avoid;   /* keep a clause together when possible */
    }
...
`;
    ...
    if (executablePath && !fs.existsSync(executablePath)) {
      console.warn(`Executable path not found: ${executablePath}, trying alternatives...`);
    ...
    console.log("Using Chromium executable:", executablePath || "Puppeteer bundled");
    ...
    try {
      ...
      const buildPageFooter = () => {
        ...
        if (signerCompanyName || signerPerson || signerOrganizationPosition) {
          ...
          second = [signerCompanyName, sanitizedPerson, signerOrganizationPosition]
            .filter(Boolean)
            .join("<br/>");
        } else {
    ...
    return NextResponse.json(
      { error: "Failed to render PDF", details: error.message || String(error) },
      { status: 500 }
    );
  }
}
