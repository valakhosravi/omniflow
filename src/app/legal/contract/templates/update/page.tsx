...
import { useSubCategories, useGetSubCategoryTemplate } from '@/packages/features/contract/hook/contractHook';
import { useDispatch } from 'react-redux';
import { setContractData, setCategoryId, setcontractTitle } from '@/packages/features/contract/slice/NonTypeContractDataSlice';
import { ContractClauseDetails, GetContractInfo, SubCategoryField } from '@/packages/features/contract/types/contractModel';
import { TemplateVariable } from '@/packages/features/contract/components/lmc/VariablesManager';
...
const convertSubCategoryFieldsToVariables = (
  ...
  return fields.map((field) => ({
    ...
    id: field.ContractFieldId ? String(field.ContractFieldId) : Date.now().toString(),
  }));
};
...
const breadcrumbs: BreadcrumbsItem[] = [
  ...
  { Name: "ویرایش قالب", Href: "/legal/contract/templates/update" },
];
...
export default function template() {
  ...
  const { subCategories, isLoading: isLoadingSubCategories } = useSubCategories(categoryId);
  
  // Get the subcategory template
  const { template: subCategoryTemplate, isLoading: isLoadingTemplate } = useGetSubCategoryTemplate(
    subCategoryId || null
  );

  const [templateLoaded, setTemplateLoaded] = React.useState(false);
  const [initialVariables, setInitialVariables] = React.useState<TemplateVariable[]>([]);
  ...
  useEffect(() => {
    if (subCategoryTemplate && subCategoryId) {
      try {
        ...
        if (subCategoryTemplate.Template) {
          ...
          if (parsed && typeof parsed === 'object' && 'ContractClauses' in parsed) {
            ...
            contractData = {
              ...
              ContractTitle: parsed.ContractTitle ?? (subCategoryTemplate.Name || ""),
              IsType: parsed.IsType ?? (subCategoryTemplate.IsPersonal || false),
              CategoryId: parsed.CategoryId ?? subCategoryTemplate.CategoryId,
              FilePath: parsed.FilePath ?? "",
              ContractFields: Array.isArray(parsed.ContractFields) ? parsed.ContractFields : [],
              ContractClauses: Array.isArray(parsed.ContractClauses) ? parsed.ContractClauses : [],
        ...
        if (subCategoryTemplate.SubCategoryFields && subCategoryTemplate.SubCategoryFields.length > 0) {
          const variables = convertSubCategoryFieldsToVariables(subCategoryTemplate.SubCategoryFields);
  ...
  }, [subCategoryTemplate, subCategoryId, dispatch, isLoadingTemplate, categoryId]);

  if (isLoading || isLoadingSubCategories || isLoadingTemplate || (subCategoryId && !templateLoaded)) {
    return (
