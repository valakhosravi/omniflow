...
export const removeInlineFontFamily = (
  ...
  let cleaned = html.replace(
    /font-family\s*:\s*['"]?IRANYekanX['"]?\s*,?\s*sans-serif\s*;?/gi,
    "",
  );

  // Also remove any standalone font-family declarations that might remain
  cleaned = cleaned.replace(
    /font-family\s*:\s*['"]?IRANYekanX['"]?\s*;?/gi,
    "",
  );
...
export const mapContentFa = (clauses: any[]): any[] => {
  return (clauses || []).map((c) => ({
    ...
    ClauseDescription: c.ClauseDescription
      ? removeInlineFontFamily(c.ClauseDescription)
      : c.ClauseDescription,
    Terms: (c.Terms || []).map((t: any) => ({
      ...t,
      Title: toFaDigits(t.Title),
      InitialDescription: removeInlineFontFamily(
        toFaDigits(t.InitialDescription),
      ),
