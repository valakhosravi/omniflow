...
export default function OrderSummary({
  ...
  const { closeOrder, isClosing } = useCloseOrder();
  const { closeOrderForOthers, isClosingForOthers } = useCloseOrderForOthers();
  const selecredUser = useBasketStore((state) => state.selectedUser);
  const router = useRouter();

  const handleCloseOrder = () => {
    const closeOrderData: CloseOrder = {
      OrderId: id,
      PaymentTypeId: 2,
    };
    ...
    if (selecredUser) {
      closeOrderForOthers({
        userId: selecredUser.UserId,
        body: closeOrderData,
      });
    } else {
      closeOrder(closeOrderData);
    }
  };
      ...
      <div className="bg-secondary-0 rounded-xl shadow-sm border border-secondary-200 p-4">
        ...
        <div className="space-y-4">
          ...
          {/* <CustomButton
            ...
            isLoading={selecredUser ? isClosingForOthers : isClosing}
          >
            پرداخت
          </CustomButton> */}
          <CustomButton
