...
import { Spinner, Tooltip } from "@/ui/NextUi";
...
export default function OrderCart({ currentDay }: OrderCartProps) {
  ...
  const { lastTransactionForOtherData, isLoading: isGettingForOthers } =
    useGetBalanceAndChargeForOthersApi(selectedUser?.UserId ?? 0);
  const clearItems = useBasketStore((state) => state.clearItems);
  const formattedDay = currentDay && currentDay.toLocaleDateString("sv-SE");
  const amount: number = useTransactionStore((state) => state.amount);
