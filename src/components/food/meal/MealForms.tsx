...
export default function MealForms({
  ...
  const { isMealLoading, mealData } = useGetMealById(mealId);
        ...
        if (imageUrl) {
          ...
          try {
            ...
            if (file) {
              ...
              console.warn("Failed to fetch image file, but continuing with edit");
              // When editing, if image fetch fails, construct proper URL for preview
              // The backend should handle the existing image
              const fullImageUrl = GetImageUrl({ bucketName: "food", path: imageUrl });
