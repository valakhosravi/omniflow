    ...
    if (processByNameAndVersion?.Data?.DefinitionId) {
      try {
        await startProcessWithPayload(
          ...
          {
            EmployeeMobileNumber: formValues.phoneNumber,
            JobPosition: formValues.jobPosition,
            EmploymentDate: toPersianDateOnly(formValues.employmentDate),
            BankId: Number(formValues.bankId),
            Amount: Number(formValues.amount.replaceAll(",", "")),
            InstallmentCount: Number(formValues.installmentCount.replaceAll(",", "")),
            InstallmentAmount: Number(
              formValues.installmentAmount.replaceAll(",", ""),
            ),
            GuaranteedNationalCode: formValues.guaranteeNationalCode,
            GuaranteedFullName: formValues.guaranteeFullName,
            HasJobPosition: formValues.hasJobPosition,
            HasPhoneNumber: formValues.hasPhoneNumber,
            HasEmploymentStartDate: formValues.hasEmploymentDate,
            IsGuarantee: formValues.isGuarantee,
          },
        );
      ...
      <div className="w-full max-w-5xl bg-white card p-8 rounded-xl">
        ...
        <section className="mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <AppInput label="نام" value={formValues.firstName} readOnly />
            <AppInput label="نام خانوادگی" value={formValues.lastName} readOnly />
            <AppInput label="نام پدر" value={formValues.fatherName} readOnly />

            <AppInput label="کد ملی" value={formValues.nationalCode} readOnly />
            <AppInput label="شماره تماس" value={formValues.phoneNumber} readOnly />
            <AppInput label="سمت شغلی" value={formValues.jobPosition} readOnly />
            <AppInput
              label="تاریخ استخدام"
              value={new Date(formValues.employmentDate).toLocaleDateString("fa-IR")}
              readOnly
            />
          </div>
        </section>
