...
export default function SalaryDeductionStartPageComponent() {
  ...
  const applicant = userDetail?.UserDetail;
  const firstName = applicant?.FirstName ?? "";
  const lastName = applicant?.LastName ?? "";
  const fatherName = applicant?.FatherName ?? "";
  const nationalCode = applicant?.NationalCode ?? "";
  const phoneNumber = applicant?.Mobile ?? "";
  const jobPosition = applicant?.Title ?? "";
  const employmentDate = applicant?.EmploymentDate ?? "";
