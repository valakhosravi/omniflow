    ...
    if (processByNameAndVersion?.Data?.DefinitionId) {
      try {
        await startProcessWithPayload(
          ...
          {
            EmployeeMobileNumber: applicant.phoneNumber,
            JobPosition: applicant.jobPosition,
            EmploymentDate: toPersianDateOnly(applicant.employmentDate),
      ...
      <div className="w-full max-w-5xl bg-white card p-8 rounded-xl">
        ...
        <section className="mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <AppInput label="نام" value={applicant.firstName} readOnly />
            <AppInput label="نام خانوادگی" value={applicant.lastName} readOnly />
            <AppInput label="نام پدر" value={applicant.fatherName} readOnly />

            <AppInput label="کد ملی" value={applicant.nationalCode} readOnly />
            <AppInput label="شماره تماس" value={applicant.phoneNumber} readOnly />
            <AppInput label="سمت شغلی" value={applicant.jobPosition} readOnly />
            <AppInput
              label="تاریخ استخدام"
              value={
                applicant.employmentDate
                  ? new Date(applicant.employmentDate).toLocaleDateString("fa-IR")
                  : ""
              }
              readOnly
            />
