...
<html lang="fa" dir="rtl">
  ...
  <body>
    <div class="container">
      ...
      <div class="content">
        اینجانب ${printableValue(data.fullName)} فرزند ${printableValue(data.fatherName)} دارای کد ملی
        ${printableValue(data.nationalCode)} و کد پرسنلی ${printableValue(data.personnelId)} در خصوص نامه کسر از حقوق / ضمانت
        شماره ${printableValue(data.trackingCode)} مورخ ${printableValue(data.createdDate)} جهت اخذ تسهیلات آقای / خانم
        ${printableValue(data.guaranteeFullName)} به مبلغ ${printableValue(data.amount)} ریال با اقساط
        ${printableValue(data.installmentAmount)} ریال اعلام می دارم در صورتی که بنا به هر دلیلی از طریق نامه مکتوب بدهی
