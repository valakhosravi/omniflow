...
export default function SectionContentPage() {
  ...
  const activeVideoProgress =
    videoProgress.sectionId === sectionId
      ? videoProgress
      : { sectionId, videoDuration: 0, watchedSeconds: 0 };
  const { videoDuration, watchedSeconds } = activeVideoProgress;
  ...
  const handleVideoLoadedMetadata = () => {
    ...
    if (savedProgress > 0) {
      watched = Math.min(savedProgress, Math.max(0, duration - 1));
      video.currentTime = watched;
      lastSavedSecondRef.current = Math.floor(watched);
    }
    setVideoProgress({ sectionId, videoDuration: duration, watchedSeconds: watched });
  };
