...
export default function SectionContentPage() {
  ...
  const lastSavedSecondRef = useRef({ sectionId: 0, second: 0 });
  ...
  const currentProgress =
    videoProgress.sectionId === sectionId
      ? videoProgress
      : { sectionId, duration: 0, watchedSeconds: 0 };
  const progressPercent =
    currentProgress.duration > 0
      ? Math.min(
          100,
          (currentProgress.watchedSeconds / currentProgress.duration) * 100,
        )
      : 0;
  ...
  const handleVideoLoadedMetadata = () => {
    ...
    if (savedProgress > 0) {
      watchedSeconds = Math.min(savedProgress, Math.max(0, duration - 1));
  ...
  const handleVideoTimeUpdate = () => {
    ...
    setVideoProgress((prev) => ({
      sectionId,
      duration:
        prev.sectionId === sectionId && prev.duration > 0
          ? prev.duration
          : video.duration || 0,
          ...
          <div>
            ...
            <div className="mt-4">
              <div className="mb-2 flex items-center justify-between text-sm/[20px] text-secondary-700">
                <span>
                  میزان مشاهده: {formatTime(currentProgress.watchedSeconds)} از{" "}
