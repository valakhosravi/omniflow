...
export default function SectionContentPage() {
  ...
  const lastSavedProgressRef = useRef({ sectionId: 0, second: 0 });
  ...
  const handleVideoTimeUpdate = () => {
    ...
    if (
      lastSavedProgressRef.current.sectionId !== sectionId ||
      currentSecond !== lastSavedProgressRef.current.second
    ) {
      localStorage.setItem(progressStorageKey, String(currentSecond));
      lastSavedProgressRef.current = { sectionId, second: currentSecond };
    }
  };
