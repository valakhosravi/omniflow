...
import { SubClauseDetails } from "../../contract.types";
import { useUpdateSubClauseMutation } from "../../contract.services";
import { addToaster } from "@/ui/Toaster";

interface LmcEditSubClauseModalProps {
  isOpen: boolean;
  onOpenChange: (open?: boolean) => void;
  subClause: SubClauseDetails | null | undefined;
  termId: number;
  onSuccess?: () => void;
}

interface EditSubClauseFormData {
  description: string;
}

export default function LmcEditSubClauseModal({
  isOpen,
  onOpenChange,
  subClause,
  onSuccess,
}: LmcEditSubClauseModalProps) {
  const {
    register,
    handleSubmit,
    formState: { errors },
    ...
    setValue,
    watch,
  } = useForm<EditSubClauseFormData>();
  const [updateSubClause, { isLoading: isUpdating }] =
  ...
  }, [subClause?.SubClauseId, reset, isOpen]);
  ...
  const onSubmit = async (data: EditSubClauseFormData) => {
    if (!subClause?.SubClauseId) return;

    try {
      await updateSubClause({
        id: subClause.SubClauseId,
        body: {
      ...
      addToaster({
        title: "تبصره با موفقیت ویرایش شد",
        color: "success",
      });
        ...
        <ModalContent className="!w-[746px] max-w-[746px] max-h-[600px] overflow-y-auto">
          ...
          <ModalBody className="px-4 py-0 space-y-4">
            <p className="font-medium text-[16px]/[30px] text-primary-950/[.5]">
              لطفا تبصره مورد نظر را بررسی کرده و در صورت نیاز اقدامات لازم را
              انجام دهید.
            </p>
