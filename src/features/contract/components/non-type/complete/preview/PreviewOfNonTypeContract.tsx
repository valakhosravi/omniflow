...
import { transformContractData } from "@/features/contract/utils/transformContractData";
import { addToaster } from "@/ui/Toaster";
import { useFullSaveMutation } from "@/features/contract/contract.services";
...
export default function PreviewOfNonTypeContract() {
  ...
  const { startProcessWithPayload } = useCamunda();

  const { contractData, CategoryId } = useSelector(
  ...
  const handleEditContract = () => {
    if (taskId) {
      router.push(
        `/issue/contract/non-type/complete?requestId=${requestId}&taskId=${taskId}&categoryId=${categoryId}`,
      );
    } else {
      router.push(`/issue/contract/non-type/complete?categoryId=${categoryId}`);
    }
  };
