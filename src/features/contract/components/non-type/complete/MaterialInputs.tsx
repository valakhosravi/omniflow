...
import { ClauseTypeSubClause, ClauseTypeTerm } from "../../../contract.types";
...
export default function MaterialInputs() {
  ...
  const handleDelete = (term: ClauseTypeTerm, sub: ClauseTypeSubClause) => {
              ...
              <div className="space-y-6" key={`term-${term.number}`}>
                {activeClause &&
                  ...
                  activeTerm && (
                    <div className="flex items-center mb-6 mt-5 justify-between">
                      <div className="font-semibold text-[14px]/[23px] text-primary-950">
                        توضیحات بند {term.number}
                      </div>
                      <CustomButton
                        buttonSize="sm"
                ...
                <div className="flex flex-col gap-y-4 w-full items-center">
                  {term.subClause.map((sub, sIndex) => (
                    <div
                      key={`sub-${term.number}-${sub.subClauseIndex}`}
