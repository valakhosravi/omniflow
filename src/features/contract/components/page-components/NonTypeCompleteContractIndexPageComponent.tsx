...
import { setTemplateFormData } from "../../contract.slices";
...
import {
  clearNonTypeAttachment,
  getNonTypeAttachment,
} from "../../utils/nonTypeAttachmentCache";
...
export default function NonTypeCompleteContractIndexPageComponent({
  ...
  const { saveSubCategoryTemplate, isLoading: isSavingTemplate } =
    useSaveSubCategoryTemplate();
  const { isLoading: isUpdatingTemplate } = useUpdateSubCategoryTemplate();
  ...
  const { template: subCategoryTemplate } =
    ...
    setTimeout(() => {
      if (modalState.data?.ClauseId) {
        ...
        const newClause: ContractClauseDetails = {
          ...
          ClauseName: formData.name,
          ClauseDescription: formData.description || "",
          SortOrder: contractData.ContractClauses.length + 1,
          Terms: [],
          IsEditable: true,
        };
        setContractData((prev) => ({
  ...
  const handleConfirmSubmit = async (settings: SignatureSettings) => {
    ...
    if (taskId) {
      ...
      try {
        ...
        const startRes = await startProcessWithPayload(
          processByNameAndVersion?.Data?.DefinitionId || "",
          camundaData,
        );

        const attachment = getNonTypeAttachment();
        if (attachment.file && startRes?.id) {
          await saveProcessAttachment({
            InstanceId: startRes.id,
            ProcessName: FeatureNamesEnum.CONTRACT,
            IsStart: true,
            AttachmentDetails: [
              {
                Title: attachment.title || "contract-attachment",
                AttachmentKey: "contractAttachment",
  ...
  const replaceVariablesWithDots = (
  ...
  return (
    <>
      <div className="container mx-auto">
        ...
        <StartProcessHeader
          title={(() => {
            const isTypeContractPage = pathname?.includes(
        ...
        <ContractActionToolbar
          ...
          onExportToPdf={
            showSubmitAndExport || isTemplatePage
              ? handleExportToPdf
              : undefined
          }
          contractClauses={contractData.ContractClauses}
