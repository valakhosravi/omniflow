...
import { Category, SaveTemplateWithFieldsFormData } from "../../contract.types";
...
export default function SelectContract({
  ...
  const { deleteSubCategoryTemplate, isLoading: isDeleting } =
    useDeleteSubCategoryTemplate();
  ...
  const [selectedPersonalContract, setSelectedPersonalContract] =
    useState<any>(null);
  ...
  const handleSelect = (contract: any) => {
    ...
    if (contract) {
      ...
      if (contract.IsPersonal === true) {
        router.push(
          `/issue/contract/non-type?categoryId=${contract.CategoryId}&subCategoryId=${contract.SubCategoryId}`,
        );
      } else {
        router.push(
          `/issue/contract/type?categoryId=${contract.CategoryId}&subCategoryId=${contract.SubCategoryId}`,
        );
      }
    }
  };
          ...
          <div className="flex gap-2">
           ...
           bg-white text-primary-950`}
              onClick={() => {
                ...
                setIsTemplate(true);
                if (router)
                  router.push(`/issue/contract/personal-template/create`);
                // onSaveTemplateOpen();
              }}
            >
              ایجاد قالب قرارداد
      ...
      {editItemId !== null && (
        <SaveTemplateWithFieldsModal
          ...
          onSave={(data: SaveTemplateWithFieldsFormData) => {
            ...
            router.push(
              `/issue/contract/templates/update?subCategoryId=${editItemId}&categoryId=${data.CategoryId}`,
            );
            onEditClose();
            setEditItemId(null);
          }}
          ...
          initialData={{
            CategoryId:
              contracts.find((c) => c.SubCategoryId === editItemId)
                ?.CategoryId || 0,
            Name:
              contracts.find((c) => c.SubCategoryId === editItemId)?.Name || "",
            Description:
              contracts.find((c) => c.SubCategoryId === editItemId)
                ?.Description || "",
          }}
