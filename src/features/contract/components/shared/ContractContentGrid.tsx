...
import { DocumentText1, SearchNormal1, Add, Copy } from "iconsax-reactjs";
...
interface ContractContentGridProps {
  ...
  onAddLibraryToContract: (library: Library) => void;
...
export default function ContractContentGrid({
  ...
  const handleLibraryClick = (library: Library, libraryType: 1 | 2) => {
    if (libraryType === 2) {
      ...
      onAddLibraryToContract(library);
    } else {
    }
  };
              ...
              <div className="flex flex-col gap-3">
                {isLoadingLibraries ? (
                  ...
                  filteredTermLibraries.map((library) => (
                    <div
                      key={`${library.TermLibraryId}-${library.LibraryType}`}
                      className="border border-primary-950/[.1] rounded-lg p-3 bg-white hover:border-primary-500 transition-all group relative"
                    >
                      <div className="flex-1">
                        <h4 className="font-semibold text-sm text-primary-950 mb-2 flex justify-between">
                          <span>{library.Title}</span>
                          <span>
                            <div className="text-xs border rounded-full px-2 py-[1px] border-primary-200 bg-primary-50 text-primary-900">
                              {library.LibraryType === 1 && "ماده"}
                              {library.LibraryType === 2 && "بند"}
                            </div>
                          </span>
                        </h4>
                      ...
                      <div className="flex justify-end gap-2">
                        <div
                          className="flex items-center justify-end gap-1 text-xs text-primary-500 border border-primary-500 p-1 rounded-lg cursor-pointer"
                          onClick={async () => {
                        ...
                        <div
                          onClick={() =>
                            handleLibraryClick(library, library.LibraryType)
                          }
                          className="flex items-center justify-end gap-1 text-xs text-primary-500 cursor-pointer"
                        >
                          <Add size={14} />
                          <span>
                            افزودن به {library.LibraryType === 2 && "ماده"}
                            {library.LibraryType === 1 && "قرارداد"}
                          </span>
                        </div>
                      </div>
                      {/* {contractData.ContractClauses.length === 0 && (
        ...
        >
          ...
          >
            ...
            <p className="text-sm text-primary-950/[.7] mb-4">
              بند &quot;{selectedLibrary.Title}&quot; را به کدام ماده اضافه
              می‌کنید؟
            </p>
