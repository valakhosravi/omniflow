...
import { GetUserInGroup } from "../invoice.type";
import { useGetUserInGroupQuery } from "../Invoice.services";
...
export default function ReferralModalAudit({
  isOpen,
  onOpenChange,
  managerDescription,
  formData,
  ...
  const { data: userInGroup } = useGetUserInGroupQuery("AUDIT-2");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await completeTaskWithPayload(taskId, {
        AuditApprove: paymentMethod === 3 ? false : true,
        AuditDescription: managerDescription,
        SecondAuditorPersonnelId: String(selectedUser?.PersonnelId),
        SecondAuditorUserId: selectedUser?.UserId,
        NewInvoiceAmount: formData.paymentAmount,
        NewAmount: formData.paymentAmount ? true : false,
